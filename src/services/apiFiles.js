...
//Llamamos a la API para listar los archivos de una crew
...
//Llama a la API para firmar la subida de un archivo en una crew
export const uploadCrewFile = async (crewId, file) => {
    try {
        //Sube el archivo a Cloudinary, devolviendo al url y el id
        const { secureUrl, publicId } = await uploadToCloudinary({
            file,
            signatureEndpoint: `${API_BASE_URL}/api/crews/${crewId}/upload-file-signature`,
        });

        //Llamamos a la API del backend para guardar el archivo con su id y su url
